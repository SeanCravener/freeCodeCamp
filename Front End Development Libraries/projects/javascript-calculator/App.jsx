...
const App = () => {
  ...
  return (
    <div>
      ...
      <span className="text-center d-block p-1 contact-text">
        Javascript Calculator made with React & Bootstrap
      </span>
      <span className="text-center d-block p-1 contact-text">
        Coded and Designed by Sean Cravener
      </span>
      <span className="text-center d-block p-1 contact-text">
        Check me out! <a href="https://github.com/SeanCravener">Github</a>{" "}
        <a href="https://codepen.io/Beelzeboss92">Codepen</a>{" "}
        <a href="https://twitter.com/SeansTheCoolest">Twitter</a>
      </span>
    </div>
  );
};
...
const BtnPad = ({
  ...
  const handleNumber = (btnValue) => {
    ...
    if (
      displayValue === "-" &&
      isOperator(equationValue[equationValue.length - 1])
    ) {
      ...
      switch (item) {
        case "+":
          equation.splice(
            i - 1,
            3,
            new Number(equation[i - 1]) + new Number(equation[i + 1])
          );
          console.log(equation);
          break;
        case "-":
      ...
      <div className="row g-0">
        ...
        <div className="d-grid col-3 p-1">
          <button
            id="equals"
            className="btn btn-primary p-2"
            onClick={() => handleEquals([...equationValue])}
            type="button"
          >
            =
          </button>
